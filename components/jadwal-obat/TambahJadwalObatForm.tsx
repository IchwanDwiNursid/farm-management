...
import { createJadwalObat } from '@/service/action'
import { JadwalObatSchema, JadwalObatType } from '@/types/input'
import { zodResolver } from '@hookform/resolvers/zod'
import React, { useEffect, useState } from 'react'
import { useForm } from 'react-hook-form'

const TambahJadwalObatForm = () => {
    ...
    const {
        ...
        register,
        setValue
    } = useForm<JadwalObatType>({
        mode: "all",
        resolver: zodResolver(JadwalObatSchema),
    })
    ...
    useEffect(() => {
        ...
        if (obat?.length) {
            setValue("obat", obat[0].id); // default obat pertama
        }
    },[obat, ayam, setValue])

    console.log("Ayam From API :", ayam)
    console.log("obat From API :", obat)

    const onSubmit = async (data: JadwalObatType) => {
        await createJadwalObat(data)
    }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="mb-3">
            <label htmlFor="obat" className="form-label">Nama Obat</label>
            <select {...register("obat")} className="form-select bg-transparent" id="obat">
                {obat?.map((item) => (
                        <option key={item.id} value={item.id}>
                            {item.nama}
                        </option>
            ...
            <div aria-live="polite" aria-atomic={true}>
                <span className="text-danger mt-2">{errors.obat?.message}</span>
            </div>
      </div>
      <div className="mb-3">
      ...
      <div className="mb-3">
            ...
            <input
                ...
                type="text"
                className="form-control bg-transparent"
                placeholder="obat harus sesuai dosis"
                id="keterangan"
                name="keterangan"
            />
            <div aria-live="polite" aria-atomic={true}>
...
export default TambahJadwalObatForm